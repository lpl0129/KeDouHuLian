...
object LifecycleConvert {
    ...
    @JvmStatic
    fun <T> bindLifecycle(observer: Observable<T>, owner: LifecycleOwner, stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP): Observable<T> = observer.compose(bind(owner, stopEvent))

    @JvmStatic
    fun <T> bindLifecycle(single: Single<T>, owner: LifecycleOwner, stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP): Maybe<T> = single.toMaybe().compose(bind(owner, stopEvent))

    @JvmStatic
    fun <T> bindLifecycleWithError(single: Single<T>, owner: LifecycleOwner, stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP): Single<T> = single.compose(bind(owner, stopEvent))

    @JvmStatic
    fun <T> bindLifecycle(maybe: Maybe<T>, owner: LifecycleOwner, stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP): Maybe<T> = maybe.compose(bind(owner, stopEvent))

    @JvmStatic
    fun <T> bindLifecycle(flowable: Flowable<T>, owner: LifecycleOwner, stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP): Flowable<T> = flowable.compose(bind(owner, stopEvent))

    @JvmStatic
    fun bindLifecycle(completable: Completable, owner: LifecycleOwner, stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP): Completable = completable.bindLifecycleWithError(owner).onErrorComplete { it is CancellationException }

    @JvmStatic
    fun bindLifecycleWithError(completable: Completable, owner: LifecycleOwner, stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP): Completable = completable.compose(bind<Nothing>(owner, stopEvent))

    private fun <T> bind(owner: LifecycleOwner, stopEvent: Lifecycle.Event): LifecycleTransformer<T> = LifecycleTransformer(LifecycleObservable(owner, stopEvent))

//    @JvmStatic
//    fun lifecycleObservable(owner: LifecycleOwner): Observable<Lifecycle.Event> = LifecycleObservable(owner)
}

internal class LifecycleObservable(private val owner: LifecycleOwner, private val stopEvent: Lifecycle.Event = Lifecycle.Event.ON_STOP)
    ...
    fun checkInActive(event: Lifecycle.Event): Boolean = stopEvent == event
