 ...
 * @date 2019/5/10 0010
 */
class BountyAllUserAdapter constructor(result: MutableList<BountyUserBean>) : RecyclerAdapter<BountyUserBean>(result) {

    override fun getLayoutIdByType(viewType: Int): Int {
        return R.layout.chat_list_item_bounty_all_memer
    }

    override fun RecyclerViewHolder.bindData(item: BountyUserBean, position: Int) {
        with(item)
        {
            addOnItemChildClickListener(jieguTv, guyongTv, jiesuanTv,shensuTv, position = position)
